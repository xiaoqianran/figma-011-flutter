...
> Configured per [Flutter AI Rules](https://docs.flutter.dev/ai/ai-rules) and
...
### Figma → Flutter workflow (mandatory)

1. **Read design first:** Use `figma-mcp-go` MCP (`get_metadata`, `get_selection`,
   `get_design_context`, `get_styles`, `get_variable_defs`, `get_fonts`) before
   writing UI code. Never guess layout from memory.
2. **Prefer structured data over screenshots:** Use `get_screenshot` only for
   visual verification after implementation.
3. **Chunk large pages:** `get_design_context(depth=2, detail="compact",
   dedupe_components=true)` then drill into sections with `get_node` /
   `scan_text_nodes`.
4. **Map tokens, not magic numbers:** Colors, typography, spacing, and radii
   must live in theme/token files under `lib/core/theme/`.
5. **Reuse components:** Repeated Figma instances → one Dart widget + data list;
   do not copy-paste card/list markup.
6. **Export assets:** Use `save_screenshots` for logos/illustrations into
   `assets/images/` and declare paths in `pubspec.yaml`.
...
lib/
  ...
  app.dart
  core/
    theme/          # AppColors, AppTextStyles, AppTheme, ThemeExtension tokens
    constants/      # design width 375, spacing, radii
    router/         # go_router routes
  shared/
    widgets/        # AppLogo, PrimaryButton, AppTextField, BottomNavBar, etc.
  features/
    splash/
    onboarding/
    auth/           # login, signup
...
assets/
  images/
  fonts/            # Merriweather, DM Sans (via google_fonts or bundled)
```
...
**Typography:** `Merriweather` (headings, buttons) + `DM Sans` (labels, body).
Use `google_fonts` package. Key sizes: H1 30/40 bold, H2 20 bold, label 16,
display 14, content 12, tab 10.

**Layout:** Design canvas 375×812. Use `SafeArea`; bottom nav height ~95;
primary button radius 10–15; cards radius 16.

### Screen implementation order

Implement in this sequence unless the user specifies otherwise:

1. Splash → 2. Onboarding (1/3, 2/3, 3/3) → 3. Log In → 4. Sign up →
5. Home → 6. History → 7. Services → 8. Nearby Courier → 9. Track Order →
10. Calculate/Ship Parcel flows → 11. Menu / Profile / FAQ / Help →
12. Chat / Notification

Use **Light Version** frames (y≈2469 row in Figma) as the default reference.

### Dependencies (prefer minimal)

- `go_router` — navigation
- `google_fonts` — Merriweather + DM Sans
- Avoid third-party state management unless requested; use `ValueNotifier` /
  `ChangeNotifier` for local/app state per Flutter rules below.

### Verification before marking a screen done

- Run `flutter analyze` with zero errors.
- All Figma copy present and in correct order.
- Colors/fonts/spacing match tokens or documented deviation.
- Assets wired in `pubspec.yaml`; no placeholder screenshots for icons.
